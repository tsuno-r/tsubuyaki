...
public class TsubuyakiController {
    ...
    // つぶやきを検索
    @GetMapping("/search")
    String showSearchResult(@RequestParam("key_word") String key_word, Model model) {

        // List<Tsubuyaki> search_list = ts.getAllTsubuyaki(); //全つぶやきを取得
        List<Tsubuyaki> search_list = ts.getSearchTsubuyaki(key_word); //キーワードを含むつぶやきを取得
        model.addAttribute("search_tsubuyakiList", search_list);   //モデル属性にリストをセット
        // model.addAttribute("tsubuyakiSearchForm", new TsubuyakiSearchForm());  //空フォームをセット
